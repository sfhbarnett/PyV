...
def PIV(image1,image2,windowsize,overlap):
    ...
    nf = mf
    BiCor = np.divide(xcorrf2(np.ones((windowsize, windowsize)),
                              np.ones((windowsize, windowsize)),mf,nf), windowsize**2)
        ...
        for ii in range(0, image1.shape[1]-windowsize//2, int((1-overlap)*windowsize)):
            if IN[int(jj+windowsize/2), int(ii+windowsize/2)] != 1:
                ...
                if np.isnan(stdcrop1) != 1 and np.isnan(stdcrop2) != 1:
                    ...
                    y1 = maxima[0]
                    x1 = maxima[1]
                    x0, y0 = intpeak(x1, y1, output[y1, x1], output[y1, x1-1], output[y1, x1+1],
                                     output[y1-1, x1], output[y1+1, x1], windowsize)
